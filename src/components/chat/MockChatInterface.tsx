...
import { useMockMatches, type Match, type ChatMessage } from "@/hooks/useMockMatches";
...
const MockChatInterface = ({ match, onBack }: ChatInterfaceProps) => {
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [newMessage, setNewMessage] = useState("");
  const [loading, setLoading] = useState(true);
  const { sendMessage, fetchMessages } = useMockMatches();
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const sentMessage = await sendMessage(match.id, newMessage);
